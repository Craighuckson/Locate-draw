...
def offset_line(x1, y1, x2, y2):
    try:
        offset = sg.Graph.draw_line((x1, y1), (x2, y2), width="1")
...
tab2layout = [
    [
        ...
        sg.Radio("RE", "roadtype", key="re"),
    ],
    [
        sg.B("Change street", key="tab1getstreet"),
        sg.T(key="tab1street"),
    ],
    [sg.B("Intersecting street:", key="tab2getstreet"), sg.I(key="tab1intersection")],
    [sg.Submit(key="sketchbuild_submit")],
]
tab1layout: list = [[]]
lcol = [
    [notify_inputmode],
...
col = [
    ...
    [
        ...
        sg.B("Read template", enable_events=True, key="read_file"),
        sg.B("Form"),
    ],
]
...
rcol = [
        ...
        [sg.Text("Object type:",), sg.Input(key='lblobject_type',)],
        [sg.Text("Coords:"), sg.Input(key='objcoords',)],
        [sg.Text("Tag:"), sg.I(key='tag',)],
        [sg.T('ID'), sg.I(key='ID',)],
        [sg.T('*PARSER')],
        [sg.Input(do_not_clear=False, key='parser_input'), sg.Submit(key='parser_submit')],
        ]


layout = [[sg.Column(lcol,element_justification='left'), sg.Menu(menu_def), sg.Column(col,element_justification='left'), sg.Column(rcol,element_justification='left')]]

def make_form_window():
    landbases = ["NORTH","SOUTH","EAST","WEST","NORTHEAST","NORTHWEST", "SOUTHEAST","SOUTHWEST","HORIZONTAL", "VERTICAL"]

    layout2  = [
        [sg.Text("Landbase:"), sg.Combo(landbases, key='landbase')],
        [sg.Text("Primary Street (Horizontal)"), sg.Input(key='street')],
        [sg.Text("Secondary Street (Vertical)"), sg.I(key='street2')],
        [sg.Submit(key='form_submit')],
    ]
